<%@ page contentType="text/html; charset=UTF-8" pageEncoding="UTF-8"%>
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
<base href="/syqzjx/">
<title>河北山叶起重机械制造有限公司</title>
<link rel="stylesheet" type="text/css" href="css/basic.css">
<link rel="stylesheet" type="text/css" href="css/qzzs.css">
</head>
	<!--顶部的flash-->
	<header>
    	<div>
    		<div id="logo"><img id="logoIMG" width="136" height="50"/></div>
        	<div id="syqz">
        		<img src="images/theme.jpg">
        	</div>
        	<div id="zyw">
            	<a href="page/qzzs.jsp"><img src="images/zh.jpg"/><span class="zh">中文版</span></a>
                &nbsp;
            	<a href="page_en/qzzs_en.jsp"><img src="images/en.jpg"/><span class="zh">English</span></a>
            </div>
       		<div id="him"><img src="images/dh.jpg"/></div>
        </div>
    </header>
    
    
    <!--菜单部分-->
    <menu>
    	<li><a href="index.jsp">首页</a></li>
        <li><a href="page/about.jsp">公司简介</a></li>
        <li><a href="page/lou.jsp">产品展示</a></li>
        <li><a href="page/news.jsp">公司动态</a></li> 
        <li><a href="page/zzzs.jsp">资质证书</a></li>
        <li><a href="page/qzzs.jsp">起重知识</a></li>
        <li><a href="page/job.jsp">人才招聘</a></li>
        <li><a href="page/contact.jsp">联系我们</a></li>
        <li><a href="page/guest.jsp">留言反馈</a></li>
    </menu>
      <!--中间动态图片-->
    <embed src="images/banner.swf" class="embed"></embed>
    <article>
    	<div class="left">
        	<img width="100%" src="images/chanpinfenlei.png"/>
        	<br/>
            <div class="tableDiv">
            <table width="80%" cellpadding="20"  id="trDiv">
               
            </table>
            </div>
        </div>
        
        <div class="right">
        	<div class="right_top">
            		<img style="margin-left:3px;" width="33" src="images/green_car.png"/>
                    <div class="right_top_left">
                	<span class="qzzsSpan">起重知识</span>
                </div>
                <div class="right_top_right">
                	<img src="images/x-tu.jpg"/>
                    <span>您当前位置:</span>&nbsp;
                    <a href="index.jsp" style="color:red;">公司首页</a>
                    <span>&nbsp;-->&nbsp;</span>
                    <a href="page/qzzs.jsp" style="color:red;">起重知识</a>
                </div>
            </div>
            <div class="right_bottom">
            </div>
        	<p id="pList"></p>	
        </div>
    
    </article>
    
    
     <!--底部版权-->
    <footer>
    	<br/><img id="logoIMG2" width="80" height="40"/>
        <p class="p1">(C)Copyright 2016 版权所有：河北山叶起重机械制造有限公司 &nbsp;地址:河北省保定市清苑区东吕工业园区 &nbsp;邮编:071100【<a href="page/login.jsp" target="_blank">企业邮箱登陆</a>】 </p>
         <p class="p2">&nbsp;&nbsp;电话：13722237047 13303028262 &nbsp;&nbsp; E-mail：hbsyqzjx@foxmail.com</p>
        <p class="p3">技术支持：<a href="#" target="_blank">gCoder工作室</a> </p>
    </footer>
 	<script type="text/javascript" src="js/jquery-2.1.4.min.js"></script>
    <script type="text/javascript" src="js/qzzs.js"></script>
    <script type="text/javascript" src="js/basic.js"></script>
<body>
</body>
</html>

